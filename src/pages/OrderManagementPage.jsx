      ...
      <Card className="border-0 shadow-sm" style={{ backgroundColor: '#ffffff', borderRadius: '16px' }}>
        <Card.Body className="p-4">
          ...
          <div className="d-md-flex justify-content-between align-items-center mb-4">
            <h2 className="fw-bold mb-3 mb-md-0" style={{ color: '#004d40' }}>Kitchen Dashboard</h2>
            
            <Nav variant="pills" activeKey={filter} onSelect={(k) => setFilter(k)} className="bg-light p-1 rounded-pill">
              <Nav.Item>
                <Nav.Link eventKey="all" className="rounded-pill px-3">ALL</Nav.Link>
              </Nav.Item>
              <Nav.Item>
                <Nav.Link eventKey="pending" className="rounded-pill px-3 text-warning-emphasis">PENDING</Nav.Link>
              </Nav.Item>
              <Nav.Item>
                <Nav.Link eventKey="preparing" className="rounded-pill px-3 text-info-emphasis">PREPARING</Nav.Link>
              </Nav.Item>
              <Nav.Item>
                <Nav.Link eventKey="completed" className="rounded-pill px-3 text-success-emphasis">COMPLETED</Nav.Link>
              </Nav.Item>
            </Nav>
          </div>
              ...
              {filteredOrders.length === 0 ? (
                ...
                filteredOrders.map(order => (
                  <tr key={order.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                    ...
                    {/* COLUMN 2: ORDER ID */}
                    <td>
                      <code className="text-muted fw-bold">
